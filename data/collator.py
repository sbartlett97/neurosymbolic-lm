...
class CognitiveCollator:
    ...
    def _process_responses(self, batch: List[dict]):
        ...
        - labels: [eos_token] - teaches model to output EOS immediately (stay silent)
        """
        responses = []
        should_respond_mask = []
        ...
        for x in batch:
            if x.get("should_respond", 0) == 1 and "response" in x and x["response"].strip():
                ...
                # For non-response samples, we'll override with EOS-only target later
                responses.append("")
